...
export function AtelierGp({
  ...
  useEffect(() => {
    ...
    const surTuile = (e: alphaTab.rendering.RenderFinishedEventArgs) =>
      tuiles.current.ajouter(e);
    const surFin = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setPistes(
        score.tracks.map((piste) => piste.name || `Piste ${piste.index + 1}`),
      );
      ...
      setErreur(
        err instanceof Error
          ? err.message
          : "Ce fichier n'a pas pu être lu comme une tablature.",
      );
    }
    ...
    const minuteur = setTimeout(async () => {
      ...
      try {
        const resultat = await preparerBande(instance, {
          ...
          onProgression: (part) =>
            vivant &&
            setEtat(`Synthèse de la bande-son… ${Math.round(part * 100)} %`),
        });
        ...
        if (vivant && (err as DOMException)?.name !== "AbortError") {
          setErreur(
            err instanceof Error
              ? err.message
              : "La bande-son n’a pas pu être produite.",
          );
        }
      }
    }, 400);
  ...
  if (!gp)
    return <ErrorText>Ce morceau n'a pas de réglages Guitar Pro.</ErrorText>;
  ...
  return (
    ...
    <div className="space-y-4">
      ...
      {/* Des colonnes tenues à la main, et non un flux qui se rééquilibre. Un empilement
         automatique range mieux, mais il redistribue tout dès qu'une carte grandit : déplier
         un réglage faisait alors sauter les cartes d'une colonne à l'autre, et l'on perdait
         des yeux celle qu'on venait d'ouvrir. Ici chaque carte a sa colonne pour de bon —
         déplier n'allonge que la sienne. */}
      <div className="grid items-start gap-4 md:grid-cols-2 xl:grid-cols-3">
        <div className="space-y-4">
          <Card className="space-y-4">
            ...
            <Field
              label="Piste"
              hint="Une seule piste donne une vidéo lisible ; toutes, un panorama."
            >
              <Select
                ...
                onChange={(e) =>
                  modifier((r) => ({
                    ...r,
                    gp: { ...gp, piste: Number(e.target.value) },
                  }))
                }
              >
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(e) =>
                  modifier((r) => ({
                    ...r,
                    gp: { ...gp, tempoPct: Number(e.target.value) },
                  }))
                }
                className="w-full accent-amber-500"
              />
              <span className="mt-1 block text-xs text-slate-500">
                Le tempo est changé dans la partition, pas dans le lecteur : le
                son, l'image et la durée annoncée le suivent tous les trois.
              </span>
            </label>
        ...
        <PanneauMiseEnScene
          video={reglages.video}
          modifier={(mutation) =>
            modifier((r) => ({ ...r, video: mutation(r.video) }))
          }
        />
        <PanneauVideo
          video={reglages.video}
...
function profilDePortee(gp: ReglagesGp): alphaTab.StaveProfile {
  if (gp.afficherPortee && gp.afficherTablature)
