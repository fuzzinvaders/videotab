    ...
    <Card className="space-y-4">
          ...
          >
            <span className="flex items-center gap-2 text-sm font-medium text-slate-200">
              <input
                ...
                onChange={() =>
                  modifier((v) => ({ ...v, disposition: choix.id }))
                }
      ...
      <Field label="Thème" hint={theme.description}>
        <Select
          ...
          onChange={(e) =>
            modifier((v) => ({ ...v, theme: themeParId(e.target.value).id }))
          }
        >
          {THEMES.map((t) => (
      ...
      {enBande ? (
        <>
          <Curseur
            ...
            onChange={(mesuresVisibles) =>
              modifier((v) => ({ ...v, mesuresVisibles }))
            }
          />
          ...
          <Repli titre="Cadrage de la bande">
            <Curseur
              ...
              affichage={
                video.margeBande === 0
                  ? "au ras"
                  : `${Math.round(video.margeBande * 100)} %`
              }
      ...
      <Field label="Encadrement">
        <Select
          value={video.cadre}
          onChange={(e) =>
            modifier((v) => ({ ...v, cadre: e.target.value as Cadre }))
          }
        >
          {CADRES.map((c) => (
      ...
      {AVEC_TRAIT.has(video.cadre) ? (
        <Curseur
          ...
          onChange={(epaisseurCadre) =>
            modifier((v) => ({ ...v, epaisseurCadre }))
          }
        />
      ) : null}
    </Card>
...
export function PanneauVideo({
  ...
  const definition =
    DEFINITIONS.find(
      (d) => d.largeur === video.largeur && d.hauteur === video.hauteur,
  ...
  return (
    <Card className="space-y-4">
      ...
      <Field
        ...
        hint="30 suffit pour un curseur ; 60 coûte le double."
      >
        <Select
          value={String(video.fps)}
          onChange={(e) =>
            modifier((v) => ({ ...v, fps: Number(e.target.value) }))
          }
        >
      ...
      >
        <Select
          value={video.cadrage}
          onChange={(e) =>
            modifier((v) => ({ ...v, cadrage: e.target.value as Cadrage }))
          }
      ...
      >
        <Select
          value={video.fond}
          onChange={(e) =>
            modifier((v) => ({ ...v, fond: e.target.value as FondVideo }))
          }
        >
      ...
      <Repli titre="Curseur">
        <Field
          label="Curseur"
          hint={STYLES.find((s) => s.id === video.curseurStyle)?.aide}
        >
          <Select
        ...
        >
          ...
          >
            <option value="glisse">
              Glissant — il suit la musique en continu
            </option>
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {video.couleur && !COULEURS.includes(video.couleur) ? (
              <button
                onClick={() =>
                  modifier((v) => ({ ...v, couleur: video.couleur }))
                }
      ...
      <Repli titre="Finitions">
        <Curseur
          ...
          pas={1}
          affichage={
            video.compteAvantSec === 0 ? "aucun" : `${video.compteAvantSec} s`
          }
          onChange={(compteAvantSec) =>
            modifier((v) => ({ ...v, compteAvantSec }))
          }
        />
        ...
        <div className="space-y-2">
          ...
          <Bascule
            ...
            onChange={(barreDeProgression) =>
              modifier((v) => ({ ...v, barreDeProgression }))
            }
          />
          <Bascule
...
function Curseur({
  ...
  return (
    <label className="block">
      ...
      />
      {aide ? (
        <span className="mt-1 block text-xs text-slate-500">{aide}</span>
      ) : null}
    </label>
  );
}
