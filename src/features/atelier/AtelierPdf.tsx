...
export function AtelierPdf({ morceau, octets }: { morceau: Morceau; octets: ArrayBuffer }) {
  ...
  const bandeSon = useMemo(
    () =>
      audio
        ? avecSilenceAvant(audio, (pdf?.decalageMs ?? 0) + reglages.video.compteAvantSec * 1000)
        : null,
        ...
        <Card className="space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="font-medium text-slate-200">
              Découpage — {pdf.systemes.length} système{pdf.systemes.length > 1 ? 's' : ''}
            </h2>
          ...
          <p className="text-sm text-slate-400">
            Tire un rectangle sur la page pour ajouter une ligne, attrape ses bords pour
        ...
        <Card className="space-y-4">
          ...
          <label className="block">
            ...
            <span className="mt-1 block text-xs text-slate-500">
              Le temps qui passe avant la première note de la bande-son — l'intro, le décompte
              du batteur, le silence du début d'enregistrement.
            </span>
          </label>

          <p className="rounded-lg bg-slate-950 px-3 py-2 text-sm text-slate-400">
            Durée calculée : <span className="text-amber-300">{formaterDuree(dureeTotaleMs(etapes))}</span>
          </p>
        </Card>
...
function Tempo({ valeur, onChange }: { valeur: number; onChange: (bpm: number) => void }) {
  ...
  function taper() {
    ...
    if (appuis.current.length > 0 && maintenant - appuis.current[appuis.current.length - 1] > 2000) {
      appuis.current = []
    }
